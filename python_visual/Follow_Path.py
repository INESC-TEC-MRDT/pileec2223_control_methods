...
bag = rosbag.Bag('/home/ze/Python/FP.bag')
...
for topic, msg, t in bag.read_messages(topics=['/unnamed_robot/sub_goal_count']):
    if topic == '/unnamed_robot/sub_goal_count':
        # Extract relevant information from the MoveGoal message
        number_points=msg.data

move_goal_x = [number_points]
move_goal_y = [number_points]
aux=0
# Iterate over the messages in the bag
for topic, msg, t in bag.read_messages(topics=['/unnamed_robot/odom', '/unnamed_robot/cmd_vel', '/unnamed_robot/move_goal', '/unnamed_robot/sub_goal_count']):
    if topic == '/unnamed_robot/odom':
        ...
        # Extract relevant information from the MoveGoal message
        move_x = msg.pose.position.x
        move_y = msg.pose.position.y

        # Append the data to the lists
        move_goal_x.append(move_x)
        move_goal_y.append(move_y)
        aux = aux+1
...
ax1.plot([move_goal_x], [move_goal_y], 'r+', markersize=10, label='move_goal')
